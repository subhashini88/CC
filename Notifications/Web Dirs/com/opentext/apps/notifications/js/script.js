function importProcess() {
  ...
  function importConfiguration() {
    $.cordys.ajax
      ({
        namespace: "http://schemas.opentext.com/apps/contractcenter/16.3",
        method: "ImportConfigurator",
        error: function (error) {
          errorToast(3000, getTranslationMessage('Configurator list import failed. Import again.'));
        },
        success: function (data) {
          if (data && data.tuple && data.tuple.old && data.tuple.old.importConfigurator &&
            data.tuple.old.importConfigurator.importConfigurator && data.tuple.old.importConfigurator.importConfigurator === "PROCESS_REGISTRATION_SHOULD_BE_FIRST") {
      ...
      ({
        ...
        success: function (data) {
          if (data && data.tuple && data.tuple.old && data.tuple.old.downloadNotificationImportReport &&
            ...
            var output = data.tuple.old.downloadNotificationImportReport.downloadNotificationImportReport;
